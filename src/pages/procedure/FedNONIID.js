...
import FTablelist from '../../components/FNTablelist';
...
export default function Process(props) {
  ...
  const begin = () =>{
    let url="ws://127.0.0.1:8000/api/getfnprocedure";//服务端连接的url
    websocket = new WebSocket(url);
    websocket.onopen = function () {
        console.log('WebSocket open');//成功连接上Websocket
        websocket.send('FednonIID');
    }
      ...
      <GridContainer>
        ...
        <Card>
            ...
            <CardBody>
                ...
                <h4 className={classes.cardbodyfont}>参与方总量:<strong>{props.location.nodes}</strong></h4>
                <h4 className={classes.cardbodyfont}>每轮参与方数量:<strong>{props.location.select_nodes}</strong></h4>
                <h4 className={classes.cardbodyfont}>数据分组为:<strong>{props.location.group}</strong></h4>
        ...
        <Card>
          <CardHeader color="info">
              <h3 className={classes.cardTitleWhite}>参与用户</h3>
          </CardHeader>
          <CardBody>
            <FTablelist select_nodes={props.location.select_nodes} size={props.location.datasize} nodes={props.location.nodes} group={props.location.group}/>
          </CardBody>
        </Card>
        </GridItem>
        <GridItem xs={5} md={5}>
