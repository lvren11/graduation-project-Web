...
import FTablelist from '../../components/FListtable';
...
export default function Process(props) {
  ...
  const begin = () =>{
    let url="ws://127.0.0.1:8000/api/getfiprocedure";//服务端连接的url
    websocket = new WebSocket(url);
    websocket.onopen = function () {
        console.log('WebSocket open');//成功连接上Websocket
        websocket.send('FedIID');
    }
    ...
    <div>
      <GridContainer>
        ...
        <Card>
          ...
          <CardBody>
            <FTablelist select_nodes={props.location.select_nodes} size={props.location.datasize} nodes={props.location.nodes} />
          </CardBody>
        </Card>
        </GridItem>
        <GridItem xs={5} md={5}>
